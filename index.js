...
app.get("/", function (req, res) {
  res.send("<h1> rental portal Project...</h1>");
});
...
app.post("/Rentproperty", async (req, res) => {
  try {
    const connection = await mongoclient.connect();
    const db = connection.db(DB);
    const Products = await db.collection("realestate").insertOne(req.body);
    res.json(Products);
    await connection.close();
  } catch (error) {
    console.log(error);
...
app.get("/Rentproperty", async (req, res) => {
  try {
    const connection = await mongoclient.connect();
    const db = connection.db(DB);
...
app.get("/Rentproperty/:id", async (req, res) => {
  try {
    const connection = await mongoclient.connect();
...
app.delete("/Rentproperty/:id", async (req, res) => {
  try {
    const connection = await mongoclient.connect();
    const db = connection.db(DB);
...
app.put("/Rentproperty/:id", async (req, res) => {
  try {
    const connection = await mongoclient.connect();
    const db = connection.db(DB);
    const productData = await db
